...
plots_agg_diff <- plots_prcp <- list()
...
for(i in 0:29){
  ...
  plots_prcp[[i + 1]] <- ggplot(test_rdr, aes(y = lat, x = lon, col = prcp)) + 
  ...
  gpm_center  <- cbind(lon = as.numeric(gravity_center_10_gpm[1]),
                       lat = as.numeric(gravity_center_10_gpm[3]))
  ...
  event_20160903_diff <- data.frame(cbind(distance = event_20160903_gpm$distance, 
                                          diff = (event_20160903_gpm$prcp - 
                                                    event_20160903_knmi$prcp)))
  event_20160903_diff <- event_20160903_diff[complete.cases(event_20160903_diff), ]
  plots_agg_diff[[i + 1]] <- ggplot(event_20160903_diff, aes(distance, cumsum(diff))) + 
    geom_point(col = "orange", alpha = 0.5) +
    geom_line(data = event_20160903_gpm, aes(distance, sum), col = "red", alpha = 0.5) +  
    ggtitle(my_date) + 
...
grid.arrange(plots_agg_diff[[2]], plots_prcp[[2]], nrow = 2)
